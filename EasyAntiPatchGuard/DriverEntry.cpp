		...
		if (*reinterpret_cast<uint32_t*>(sections_array[i].Name) == 0x7865742E) {
			...
			for (int i = 0; i < scan_size - 10; i++) {
				...
				if (p1 == 0x1D8B4C && p2 == 0xC0854800) {
					...
					je      ret
					cmp     qword ptr ds:[rax], 0x48513148   //     KiDpcDispatch 
					je      ret
					cmp     qword ptr ds:[rax], 0x11111111   //		KiTimerDispatch[0]
					jne     ok
					...
					unsigned char patch_code[] = {
						0x48, 0x81, 0x38, 0x2E, 0x48, 0x31, 0x11, 0x74, 0x1C, 0x48, 0x81, 0x38, 0x48, 0x31, 0x51, 0x48,
						0x74, 0x13, 0x48, 0x81, 0x38, 0x11, 0x11, 0x11, 0x11, 0x75, 0x0B, 0x48, 0x81, 0x78, 0x04, 0x22,
						0x22, 0x22, 0x22, 0x75, 0x01, 0xC3, 0xFF, 0xE0
					};
					*(uint32_t*)(&patch_code[0x15]) = KiTimerDispatch[0];
					*(uint32_t*)(&patch_code[0x1F]) = KiTimerDispatch[1];
